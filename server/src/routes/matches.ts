...
matchesRouter.get("/", async (req, res) => {
  const matches = await prisma.match.findMany({
    ...
    include: {
      ...
      _count: { select: { messages: { where: { senderId: { not: req.userId }, readAt: null, deletedAt: null } } } },
    },
...
matchesRouter.delete("/:matchId", async (req, res) => {
  ...
  await prisma.$transaction([
    prisma.match.update({ where: { id: match.id }, data: { status: "UNMATCHED", unmatchedAt: new Date() } }),
    prisma.typingIndicator.deleteMany({ where: { matchId: match.id } }),
  ]);
  res.status(204).send();
});
