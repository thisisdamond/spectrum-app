...
export function SettingsScreen() {
  ...
  return (
    <Screen>
      ...
      <Card><AccessibleText variant="bodyLarge" weight="800">Discovery</AccessibleText><Button label={profile?.discoveryPaused ? "Resume discovery" : "Pause discovery"} variant="secondary" onPress={() => void saveProfile({ discoveryPaused: !profile?.discoveryPaused }, true)} /><Button label={settings.breakModeUntil ? "End break" : "I need a 24-hour break"} variant="secondary" onPress={() => update({ breakModeUntil: settings.breakModeUntil ? null : new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString() })} /></Card>
